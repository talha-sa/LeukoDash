...
def show():
    ...
    if df is not None and not df.empty:
        ...
        st.markdown("Assign samples to groups (e.g. ALL vs AML) for comparison. Skip if not needed.")
        ...
        st.markdown("Shows top variable genes across samples. Hover over any cell for exact value.")
            ...
            with st.spinner(f"Running {dr_method}... ⏳"):
                ...
                if dr_method == "PCA":
                    ...
                    axis_labels = {
                        ...
                        "y": f"PC2 ({explained[1]:.1f}% variance)"
                    }
                else:
                    # t-SNE needs PCA first to reduce dimensions
        ...
        if "dr_plot_df" in st.session_state:
            ...
            dr_method = st.session_state["dr_method"]
            ...
            fig_dr = px.scatter(
                ...
                color=color_col,
                hover_name="Sample",
                title=f"{dr_method} Plot — Sample Clustering",
                labels=axis_labels,
                template="plotly_white",
                height=500,
            ...
            if dr_method == "PCA":
                st.info("💡 Each dot = one patient sample. Samples that cluster together have similar gene expression patterns.")

        st.markdown("---")

        # ════════════════════════════════════════
        # SECTION 4: Group Comparison (if groups defined)
        # ════════════════════════════════════════
        if use_groups and group1_cols and group2_cols:
            st.subheader(f"⚖️ {group1_name} vs {group2_name} Comparison")
            st.markdown("Average expression of top genes in each group side by side.")
